...
stockTable[!complete.cases(stockTable),]

len = nrow(stockTable)
AdjClose = as.double(stockTable[len][1,6])
...
#library(RMySQL)
#library(DBI)
#conn <- dbConnect(MySQL(), dbname = "StockPredict", username="root", password="tyanmysql", host="127.0.0.1",port=3306)
...
as.double(unscale(predict(nn, Tdata.train[nrow(Tdata.train),]), norm.data))
AdjClose
